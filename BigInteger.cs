...
namespace Big
{
    ...
    public class BigInteger
    {
        ...
        //Returns a float of the division of two BigIntegers.
        public static float Divide(BigInteger leftSide, BigInteger rightSide, int deci)
        {//leftSide = 5123, rightSide = 12345;
            string ls = leftSide.ToString();
            string rs = rightSide.ToString();
            int ll = ls.Length; //4
            int rl = rs.Length; //5
            if(ll > Constant.PRECISION.ToString().Length)
                ls = ls.Substring(0, Constant.PRECISION.ToString().Length);//"5123"
            if (rl > Constant.PRECISION.ToString().Length)
                rs = rs.Substring(0, Constant.PRECISION.ToString().Length);//"1234"
            float lf = float.Parse(ls) / Constant.PRECISIONF;//5.123
            float rf = float.Parse(rs) / Constant.PRECISIONF;//1.234
            if(ll > rl)
            {
                lf = lf * Mathf.Pow(10f, ll - rl);
            }
            else
            {
                rf = rf * Mathf.Pow(10f, rl - ll);
            }
            return lf / rf;
        }
        ...
        public static double ToDouble(BigInteger value)
        {
            ...
            return System.Double.Parse(value.ToString(), System.Globalization.NumberStyles.Integer, System.Globalization.CultureInfo.CurrentCulture);
        }
